...
class GentooPackageManager:
    ...
    def _setup_package_use(self):
        ...
        try:
            ...
            # package_use may be written either as a YAML map:
            #     package_use:
            #       "cat/pkg": "flag1 flag2"
            ...
            if isinstance(self.package_use, dict):
                ...
                entries = {}
                for item in self.package_use:
                    if isinstance(item, dict):
                        entries.update(item)
                    else:
                        libcalamares.utils.warning(f"Ignoring malformed package_use entry: {item!r}")
            else:
                libcalamares.utils.warning("package_use must be a map or a list of maps; skipping USE flag setup")
                return
            ...
            with open(use_file, 'w') as f:
                ...
                for package, flags in entries.items():
                    f.write(f"{package} {flags}\n")

            libcalamares.utils.debug(f"Created {use_file} with {len(entries)} entries")
        except Exception as e:
            libcalamares.utils.warning(f"Could not setup package.use: {e}")
    
    def _setup_accept_keywords(self):
        """Setup package.accept_keywords file for testing/unstable packages.

        Each entry may be a bare atom ("dev-lang/rust"), where the keyword
        defaults to "~amd64" for backward compatibility; a full "<atom> <keyword>"
        string ("dev-lang/rust ~arm64"), used verbatim; or a single-entry map
        ("<atom>": "<keyword>"). Written to
        /etc/portage/package.accept_keywords/00-calamares.accept.keywords.
        """
        keywords_dir = "/etc/portage/package.accept_keywords"
        keywords_file = os.path.join(keywords_dir, "00-calamares.accept.keywords")
        ...
        try:
            ...
            lines = []
            for entry in self.accept_keywords:
                if isinstance(entry, dict):
                    lines.extend(f"{atom} {keyword}" for atom, keyword in entry.items())
                elif isinstance(entry, str):
                    # a bare atom keeps the historical ~amd64 default;
                    # an "<atom> <keyword>" string is used as is
                    lines.append(entry if len(entry.split()) > 1 else f"{entry} ~amd64")
                else:
                    libcalamares.utils.warning(f"Ignoring malformed accept_keywords entry: {entry!r}")
            ...
            libcalamares.utils.debug(f"Created {target_keywords_file} with {len(lines)} entries")
        except Exception as e:
